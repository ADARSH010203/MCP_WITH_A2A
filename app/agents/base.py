    ...
    async def stream(self, query: str, session_id: str) -> AsyncIterable[dict[str, Any]]:
        ...
        for item in self.graph.stream({"messages": [("user", query)]}, config, stream_mode="values"):
            ...
            if isinstance(message, ToolMessage) or (isinstance(message, AIMessage) and message.tool_calls):
                yield {"is_task_complete": False, "require_user_input": False, "content": self.processing_message}
    ...
    def get_agent_response(self, config: dict[str, Any]) -> dict[str, Any]:
        ...
        if isinstance(response, ResponseFormat):
            if response.status == "completed":
                return {"is_task_complete": True, "require_user_input": False, "content": response.message}
            if response.status in {"input_required", "error"}:
                return {"is_task_complete": False, "require_user_input": True, "content": response.message}
